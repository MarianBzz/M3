...
async function problemB () {
  ...
  // AsyncAwait version
  const resp = await promisifiedReadFile('poem-one/stanza-02.txt');
  const respu = await promisifiedReadFile('poem-one/stanza-03.txt');
  blue(resp, respu);
...
async function problemC () {
  ...
  const responsei = await promisifiedReadFile('poem-one/stanza-03.txt');
  blue(response);
  blue(responsei);
  blue("done");
}
