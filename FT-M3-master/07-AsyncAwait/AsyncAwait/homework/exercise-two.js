...
async function problemA () {
  ...
  // AsyncAwait version
  async function comoVosQuieras(fileName) {
    blue(await promisifiedReadFile(fileName))

  }

  await Promise.all([comoVosQuieras('poem-two/stanza-01.txt'), comoVosQuieras('poem-two/stanza-02.txt')])
  console.log("done")
}
...
async function problemB () {
  ...
  const newArray = filenames.map(p => comoVosQuieras(p))
...
async function problemC () {
  ...
  for (let i = 0; i < filenames.length; i++) {
    await comoVosQuieras(filenames[i]);
  }
